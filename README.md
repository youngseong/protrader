...
An async Rust trading bot for the Korean stock market (KOSPI) via the [Korea Investment & Securities (KIS) Open API](https://apiportal.koreainvestment.com). Implements an **Opening Range Breakout (ORB)** strategy.

## Strategy

The bot captures the price range (high/low) during the first N minutes of the session, then enters a long position when price breaks out above the range. Positions are exited on stop-loss, daily loss limit, or end-of-day forced close.

**Session lifecycle (KST)**

```
09:00 → CapturingRange  — tracks high/low for each symbol
09:30 → Monitoring      — watches for breakout entries and stop-losses
...
breakout_buffer_pct = 0.2  # % above range high to trigger buy
...
|---|---|
| `src/strategy.rs` | Pure synchronous signal logic; no I/O. `on_tick()` emits signals, `record_buy/exit()` mutates state after order confirmation. |
