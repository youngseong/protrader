...
use protrader::order::{LiveOrderClient, OrderClient, PaperOrderClient};
...
async fn main() -> anyhow::Result<()> {
    ...
    let order_client: Arc<dyn OrderClient> = match config.trading.mode {
        TradingMode::Paper => {
            tracing::info!("Running in PAPER mode — no real orders will be placed");
            Arc::new(PaperOrderClient)
        }
        TradingMode::Live => {
            tracing::info!("Running in LIVE mode — real orders WILL be placed");
            Arc::new(LiveOrderClient::new(credentials).await?)
        }
    };
